...
3. If this completion begins or continues an away block for other unlocked modes with existing timed records, add at most one `rest` entry per away block to each such non-active mode. The first rest uses the slowest retained real round. Later rests construct progressively faster paces from non-rest item timings: start at the slowest item, skip one timing for the second rest, then skip two, three, and so on, capping selection at the median item. Normalize that item-derived pace between the mode's slowest and median retained real round so rests approach—but do not pass—the central real score. Stop adding rests once the sequence reaches that median or cannot produce a strictly faster rest. For example, a block of several 3-way rounds after 2-way play still gives 2-way at most one rest, not one per 3-way round.
4. A round-level rest uses the sequence described above for time and the highest current mistake count. Timed actual and temporary calibration entries still present can establish the real-round endpoints; prior rests and bet-weighted duplicates cannot establish those endpoints.
...
Heart loss still needs a clear safety line.  Use the median of the stored whole-round timing records, including real completed runs, overhead-adjusted first-round calibration pseudo-scores, rest entries, and bet-weighted duplicate entries.  The actual first-round calibration entry remains separate until it is eventually retired after the derived temporary pseudo-scores.  This raw median should usually be more lenient than the meta-percentile Club targets when the player is improving, buying upgrades, and collecting rests.
