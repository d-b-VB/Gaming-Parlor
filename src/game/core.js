...
export function addRoundMemory(state, modeId, timeSeconds, createdAt, mistakes = 0) { const next = structuredClone(state); next.gameMemory ??= {}; next.gameMemory[modeId] ??= { entries: [] }; const entries = next.gameMemory[modeId].entries ?? []; entries.push({ timeSeconds, mistakes, entryType: 'actual', createdAt, percentileAtRun: percentileAtRun(timeSeconds, entries) }); next.gameMemory[modeId].entries = entries.slice(-20); for (const other of Object.keys(next.unlockedModes)) { if (other === modeId || !next.unlockedModes[other]) continue; next.gameMemory[other] ??= { entries: [] }; const otherEntries = next.gameMemory[other].entries ?? []; const worst = otherEntries.length ? Math.max(...otherEntries.map((entry) => entry.timeSeconds)) : MODES[other].heartSafetySeconds; otherEntries.push({ timeSeconds: worst, mistakes: 0, entryType: 'rest', createdAt }); next.gameMemory[other].entries = otherEntries.slice(-20); } return next; }
...
export function settleRound(state, modeId, timeSeconds, mistakes, seed, createdAt = new Date().toISOString()) { const entries = state.gameMemory?.[modeId]?.entries ?? []; const safety = heartSafety(modeId, entries); const actual = entries.filter((entry) => entry.entryType === 'actual').map((entry) => entry.timeSeconds); const newWorst = actual.length > 0 && timeSeconds > Math.max(...actual); const timeHeartsLost = timeSeconds > safety ? (newWorst ? 2 : 1) : 0; const mistakeResult = mistakePressure(entries, mistakes); const heartsLost = timeHeartsLost + mistakeResult.heartsLost; const basePayout = Math.max(0, payoutScore(state, modeId) + (heartsLost ? -1 : 0)); const activeBet = state.activeClubBet?.modeId === modeId ? state.activeClubBet : null; const betWinnings = activeBet && timeSeconds <= activeBet.targetSeconds ? activeBet.stake + Math.floor(activeBet.stake * activeBet.oddsMultiplier) : 0; const next = addRoundMemory(state, modeId, timeSeconds, createdAt, mistakes); next.resources.hearts = Math.max(0, next.resources.hearts - heartsLost); next.resources.diamonds += basePayout + betWinnings; next.activeClubBet = null; next.eventLog = [...(next.eventLog ?? []), { type: 'round', modeId, timeSeconds, mistakes, seed, createdAt, diamondsDelta: basePayout + betWinnings, heartsDelta: -heartsLost, timeHeartsLost, mistakeHeartsLost: mistakeResult.heartsLost, medianMistakes: mistakeResult.medianMistakes, maxMistakes: mistakeResult.maxMistakes, betWinnings }].slice(-50); return next; }
