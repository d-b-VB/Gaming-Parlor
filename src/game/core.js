...
export function estimateTargets(modeId, entries) { const mode = MODES[modeId]; const actualCount = entries.filter((entry) => entry.entryType !== 'rest').length; const fallbackEven = mode.starterTargets[0].timeSeconds; const fallbackFactors = { half: 1.2, even: 1, double: 0.8, five: 0.65, ten: 0.55 }; if (entries.length < 5) return BET_TIERS.map((target) => ({ ...target, timeSeconds: Math.max(1, Math.round(fallbackEven * fallbackFactors[target.id])), available: actualCount >= target.minHistory, actualCount })); const sorted = entries.map((entry) => entry.timeSeconds).sort((a, b) => a - b); return BET_TIERS.map((target) => ({ ...target, timeSeconds: Math.max(1, Math.round(quantile(sorted, target.chance))), available: actualCount >= target.minHistory, actualCount })); }
