...
export function firstRoundCalibrationScores(totalTimeSeconds, itemTimes, totalItems) {
  ...
  const half = itemTimes.slice(Math.floor(itemTimes.length / 2));
  const quarter = itemTimes.slice(Math.floor(itemTimes.length * 0.75));
  const candidates = [
    ...
    ['exp_weighted_item_pace', expWeightedMean(itemTimes) * totalItems],
    ['median_last_half_item_pace', median(half) * totalItems],
    ['midhinge_item_pace', midhinge(itemTimes) * totalItems],
    ['modal_item_pace', narrowestWindowMode(itemTimes) * totalItems],
    ['modal_second_half_item_pace', narrowestWindowMode(half) * totalItems],
  ];
  const byScore = new Map();
  for (const entry of candidates.map(([source, score]) => ({ source, sources: [source], timeSeconds: roundScore(score) })).filter((entry) => entry.timeSeconds <= totalTimeSeconds)) {
    const key = entry.timeSeconds.toFixed(2);
    if (byScore.has(key)) byScore.get(key).sources.push(entry.source);
    else byScore.set(key, entry);
  }
  return [...byScore.values()].map((entry) => ({ ...entry, source: entry.sources.join('+') }));
}
