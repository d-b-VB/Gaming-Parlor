...
const TARGET_RECENT_ACTUAL_LIMIT = 100;
const CLUB_COST = 1;
...
export function estimateTargets(modeId, entries) { const mode = MODES[modeId]; const actualEntries = entries.filter((entry) => entry.entryType !== 'rest'); const actualCount = actualEntries.length; const recentActual = actualEntries.slice(-TARGET_RECENT_ACTUAL_LIMIT); const fallbackEven = mode.starterTargets[0].timeSeconds; const fallbackFactors = { half: 1.2, even: 1, double: 0.8, five: 0.65, ten: 0.55 }; const mistakeHistory = recentActual.filter((entry) => Number.isFinite(entry.mistakes)).map((entry) => entry.mistakes).sort((a, b) => a - b); const mistakeLimitFor = (target) => mistakeHistory.length ? Math.max(0, Math.floor(quantile(mistakeHistory, target.chance))) : 0; if (actualCount < 5) return BET_TIERS.map((target) => ({ ...target, timeSeconds: Math.max(1, Math.round(fallbackEven * fallbackFactors[target.id])), mistakeLimit: mistakeLimitFor(target), available: actualCount >= target.minHistory, actualCount })); const sorted = recentActual.map((entry) => entry.timeSeconds).sort((a, b) => a - b); return BET_TIERS.map((target) => ({ ...target, timeSeconds: Math.max(1, Math.round(quantile(sorted, target.chance))), mistakeLimit: mistakeLimitFor(target), available: actualCount >= target.minHistory, actualCount })); }
