...
export function firstRoundCalibrationScores(totalTimeSeconds, itemTimes, totalItems) {
  ...
  ].map(([source, score]) => ({ source, timeSeconds: roundScore(score) })).filter((entry) => entry.timeSeconds <= totalTimeSeconds);
  const scores = [{ source: 'actual_first_round', sources: ['actual_first_round'], timeSeconds: roundScore(totalTimeSeconds) }];
  if (derivedCandidates.length) {
    const aggregate = roundScore(median(derivedCandidates.map((entry) => entry.timeSeconds)));
    scores.push({ source: 'first_round_calibration_median', sources: derivedCandidates.map((entry) => entry.source), timeSeconds: aggregate });
  }
...
function realRoundEntries(entries) { return entries.filter((entry) => entry.entryType === 'actual' && !entry.weightedByBet && (!entry.calibrationSource || entry.calibrationSource === 'actual_first_round')); }
function heartBenchmarkEntries(entries) { return entries.filter((entry) => entry.entryType === 'actual' && !entry.weightedByBet && (!entry.calibrationSource || entry.calibrationSource === 'actual_first_round' || entry.calibrationSource === 'first_round_calibration_median')); }
