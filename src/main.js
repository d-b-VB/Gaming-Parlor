...
function barsHtml(safety, activeBet, hideTimers = false) {
  ...
  const heartLabel = hasHeartLimit ? `<span>${heartsHtml()} Heart safe ${fmt(Math.max(0, safety - elapsed))}</span><span>${fmt(safety)}</span>` : `<span>${heartsHtml()} First run: no Heart timer</span><span>Take your time</span>`;
